describe('Login Page', () => {
    ...
    beforeEach(() => {
      cy.visit('http://localhost:8000/login');  // Adjust URL if necessary
    });
    ...
    // 6. Test Spinner Visibility on Submit
    it('should show a spinner on submit', () => {
      cy.get('input[name="username"]').type('testuser');
      cy.get('input[name="password"]').type('testpassword');
      cy.get('button[type="submit"]').click();
      cy.get('.spinner').should('be.visible');
      cy.wait(1500);  // Simulate waiting
